...
//-------------------------------->metier product(book,movie,voiceover,cosplay)
...
public function addWishlist($iduser,$idproduct) {
  try {
      $sql = "INSERT INTO wishlist (iduser,idproduct) VALUES (:iduser,:idproduct)";
      $stmt = $this->pdo->prepare($sql);
...
public function deleteWishlist($iduser,$idproduct) {
  try {
      $stmt = $this->pdo->prepare("DELETE FROM wishlist WHERE iduser = :iduser and idproduct = :idproduct");
      $stmt->bindValue(':iduser', $iduser);
      $stmt->bindValue(':idproduct', $idproduct);
      $stmt->execute();
      return true;
...
public function getUserWishlist($iduser) {
  try {
      $sql = "SELECT idproduct FROM wishlist WHERE iduser = :iduser";
      $stmt = $this->pdo->prepare($sql);
      $stmt->bindValue(':iduser', $iduser);
      $stmt->execute();
      $wishlist = array();
      while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
          $wishlist[] = $row['idproduct'];
      }
      return $wishlist;
  } catch (PDOException $e) {
      echo "Error: " . $e->getMessage();
  ...
  //other functions
   ...
   public function get_categorySatistic()
   {
       try {
   ...
   public function modifyCategoryLikes($categoryName, $action)
   {
       try {
           // Get the current likes count for the category
           $sql = "SELECT likes FROM category WHERE namecategory = :namecategory";
           $stmt = $this->pdo->prepare($sql);
           $stmt->bindValue(':namecategory', $categoryName);
           $stmt->execute();
           $likesCount = $stmt->fetchColumn();
   
           // Modify the likes count based on the action
           if ($action == 'inc') {
               $likesCount++;
           } else {
              if($likesCount>0)
           ...
           $sql = "UPDATE category SET likes = :likes WHERE namecategory = :namecategory";
           $stmt = $this->pdo->prepare($sql);
           $stmt->bindValue(':likes', $likesCount);
           $stmt->bindValue(':namecategory', $categoryName);
           $stmt->execute();
