...
{
    if(!empty($_POST['idevent']) && !empty($_POST['nbpersonnes']) && !empty($_POST['prixtotal']))
    {
		$idevent = $_POST['idevent'];
		$nbpersonnes = $_POST['nbpersonnes'];
		$prixtotal = $_POST['prixtotal'];
		$Reservation = new Reservation (NULL, $idevent, $nbpersonnes , $prixtotal);
        if(!empty($_POST['updating']))
        {
            $id=$_POST['updating'];
			$Reservation->setIdReservation($id);
		  $res=$reservationC->updateReservation($Reservation);
...
$reservations=$reservationC->displayReservation();
			...
			<main class="content ">
				<div class="d-flex align-items-center justify-content-between mb-3" >
					 ...
					 <div id="popup-container-Admin" class="popup-container-Admin" style="width: 28%;">
						<a href="reservation.php" id="close-popup-Admin"><i class="fas fa-times"></i></a>
						<div id="popup-content-Admin" class="">
							
							<h4 class="model-titel">Add a new Category </h4>
							<form action="reservation.php" method="post" style="height: 330px;">
							    <label for="idevent">Event ID:</label>
                                <input type="number" id="idevent" name="idevent" required><br>

                                <label for="nbpersonnes">Number of People:</label>
                                <input type="number" id="nbpersonnes" name="nbpersonnes" required><br>

                                <label for="prixtotal">Total Price:</label>
                                <input type="number" id="prixtotal" name="prixtotal" required><br>
								<div class="d-flex align-items-center justify-content-between mb-3  ">
									<a href="reservation.php" class="btn btn-secondary btn-block mt-3" style="border-radius: 5px;">Cancel</a>
							...
							<div class="card flex-fill">
								...
								<table class="table table-hover my-0">
									...
									<?php foreach ($reservations as $reservation): ?>
                                    	<tr>
                                    		<td   >
                                    			<span class="badge bg-danger" ><?php echo $reservation["idreservation"]; ?></span>
                                    		</td>
											<td   >
                                    			<span><?php echo $reservation["idevent"]; ?></span>
                                    		</td>
											<td   >
                                    			<span  ><?php echo $reservation["nbpersonnes"]; ?></span>
                                    		</td>
											<td   >
                                    			<span class="badge bg-success" ><?php echo $reservation["nbpersonnes"]; ?></span>
                                    		</td>
                                    		<td>
                                    			 <a href="reservation.php?update=<?= $reservation["idreservation"]; ?>" style="margin:10px;" class="btn btn-primary open-popup-ElseWhere" name="modify" id="update-button">Modify</a>
                                    			 <a href="reservation.php?delete=<?= $reservation["idreservation"]; ?>" style="margin: 10px;" class="btn btn-danger" name="delete">Delete</a>
                                    		</td>
                                    	</tr>
                                    <?php endforeach; ?>                         
									</tbody>
...
if(isset($_GET['update']))
{
 ...
 echo '<script>
	...
	modelTitel.innerHTML="Update a Reservation";
